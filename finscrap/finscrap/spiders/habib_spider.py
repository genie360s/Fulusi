    ...
    def parse(self, response):
        ...
        for exchange_rate_data in exchange_rate_datas:
            yield {
                ...
                'buying_rate': float(exchange_rate_data[1].strip().replace(',','') if exchange_rate_data[1] else None),
                'selling_rate': float(exchange_rate_data[2].strip().replace(',','')  if exchange_rate_data[2] else None),
            }